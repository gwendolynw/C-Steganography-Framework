...
int main(int argc, const char * argv[])
{
    ...
    printf("determine message_size is %i and carrier_size is %i\n", message_size, carrier_size);
	...
	if(doEmbed){
		...
		while (*message && current_ucb_index <= carrier_size){
			...
			if (!index_to_change) continue; //if zero, don't do any embedding
			
			index_to_change--; //because array is zero-indexed
		    
			//embed the secret message: change message at index of sum
            //because we only know how far along we are in the ucb array in relative to codeword_size (as in, we're only incrementing by n)
			//we need to figure out the index to change relative to that factor of n

		    (UCB[current_ucb_index-(codeword_size - index_to_change)]>0)?UCB[current_ucb_index-(codeword_size - index_to_change)]--: UCB[current_ucb_index-(codeword_size - index_to_change)]++;
		    ...
		    if (UCB[current_ucb_index-(codeword_size - index_to_change)] == 0){
        ...
        while (message_index < message_length){
            ...
            for (current_hash_bit = 0; current_hash_bit < message_partition_size; current_hash_bit++){
                if (message_bit_index > 7){ //we reach end of byte, so increment to next byte of allocated array
                    message_bit_index = 0;
                    printf("here is newly minted number %i\n", extracted_message[message_index]);
                    message_index++;
                    extracted_message[message_index] = 0;
                }
    ...
    else printf("CAN'T EXTRACT MESSAGE WASN'T ENCODED");
}
